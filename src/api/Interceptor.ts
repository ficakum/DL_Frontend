...
export const Interceptor = (axios: AxiosInstance) => {
  ...
  const handleUnauthorizedErrors = (responseError: AxiosError) => {
    ...
    if (error === INVALID_TOKEN && error_description.includes(INVALID_ACCESS_TOKEN_DESCRIPTION)) {
      return handleExpiredAccessToken(responseError.config)
    }

    if (error === INVALID_TOKEN && error_description.includes(INVALID_REFRESH_TOKEN_DESCRIPTION)) {
